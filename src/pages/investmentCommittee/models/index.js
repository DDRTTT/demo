...
export default {
  ...
  effects: {
    ...
    *handleSearch({payload, val, callback}, {call, put}) {
      ...
      if (response && response.status === 200) {
        yield put({
          type: 'saveSearch',
          payload: response.data.committeeType
        });
        if (callback) callback(response);
      } else {
    ...
    *handleProName({payload, val, callback}, {call, put}) {
      const response = yield call(handSearchAPI, payload, val);
      if (response && response.status === 200) {
        yield put({
          type: 'selectProName',
  ...
  reducers: {
    ...
    selectProName(state, { payload }) {
      return {
        ...state,
        selectProName: payload,
      };
    },
  },
};
