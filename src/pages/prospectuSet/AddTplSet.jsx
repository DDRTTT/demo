...
import { Modal, Form, Input, Button, Select, Space, Upload, Radio, message, Tooltip, Row, Col } from 'antd';
import { getCustodianList, getProductList, getTplInfo, copyTemplate, getProList, baseCompare, saveTemplate, getNginxIP, getTplList  } from '@/services/prospectuSet';
import {
  getFilePathByCode,
...
const Add = forwardRef((
  ...
  const showModal = () => {
    ...
    getProList({queryParams: [{type: 0, code: "id", required: 1, value: ""}]}).then((res)=>{
      if(res.status === 200) {
  ...
  const onValuesChange = (a, formItemProps) => {
   if (formItemProps['productName'] && formItemProps['productName'].length > 0)
    {
  ...
  const onAdd = () => {
    form.validateFields().then(formVals=>{
      copyTemplate({ proCode: formVals.productName }).then(res=>{
        if (res.status === 200) {
  ...
  const onRefresh = (info={}) => {
    ...
    if(template.length > 0) {
      template.forEach((item, index) => {
        ...
        baseBook.push(item.substring(num+1).trim());
        compareData.push({key:item.trim(), title:item.trim(), article: item.substring(num+1).trim()});
      })
    }
    if(current.length > 0) {
      current.forEach((item, index) => {
        const num = item.indexOf('、');
        newBook.push(item.substring(num+1).trim());
        currentTarget.push({key:item.trim(), title:item.trim(), article: item.substring(num+1).trim()});
      })
    }
    ...
    const newNowData = newBook.filter(item=>{
      return !baseBook.includes(item.trim())
    });
          ...
          <Space>
            ...
            >
              <Button
                ...
                disabled = {uploadBtnLoading || disableUpload }
              >
                导入模板
              </Button>
            </Upload>
