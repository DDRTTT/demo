...
const HandleAdd = forwardRef((
  ...
  const jumpPage = (item, status) => {
    ...
    router.push(`/contract/prospectusConfig/edit?title=招募说明书设置`);
  };
  ...
  return (<>
      ...
      <Modal title="招募说明书" footer={null} width={800} visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
        <Form {...layout} form={form}>
          <Form.Item label="托管人" name='custodian' >
            <Select
            placeholder="请选择"
            onChange={onCustodianChange}
            allowClear>
              {custodianList.map(item => <Option value={item.id} key={item.id}> {item.orgName} </Option>)}
            </Select>
          </Form.Item>
          <Form.Item label="产品名称" name='productName' rules={[{ required: true }]}>
            ...
            >
              {productList.map(item => <Option value={item.proCode} key={item.proCode}>{item.proName}</Option>)}
            </Select>
          </Form.Item>
