...
import { Popconfirm, Checkbox, Input, Row, Col,
  Button, Table, Tabs, Card, Spin, Empty, Divider,
  Modal, Switch, Breadcrumb, Tree, Space, Tag,
  message,Tooltip,Form,
} from 'antd';
...
import { getBusinessData, getMapping, getNginxIP, saveTpltags, saveTemplate, saveTagContent, setAuthorize } from '@/services/prospectuSet';
...
class Index extends Component {
  ...
  oneClickConfig = () => {
    ...
    setAuthorize({fileSerialNumber:templateKey}).then(res => {
      if (res.status == 200) {
  ...
  render() {
    ...
    return (
      <div >
        ...
        {
          _status === 'isSee' ?
          <div style={{ width: '100%', float: 'left', minHeight: '800px' }}>
            ...
            <div style={{ backgroundColor: '#fff' }}>
              <div style={{ textAlign: 'right' }}>
                <Space>
                  ...
                  <Button onClick={this.oneClickConfig} disabled={this.state.configFlag} loading={configLoading}>一键配置</Button>
                ...
                <Tabs defaultActiveKey="1" onChange={this.tabsChange} size="small">
                  ...
                  <TabPane tab={'标签映射关系设置'} disabled={!switchStatus} key="2">
                    ...
                    <Search
                      ...
                      loading={docxTagsLoading}
                    />
                    <div className={styles.tagsBox2+' '+styles.tagsBox} style={{marginBottom: '5px',paddingLeft: '20px'}}>
                      {docxTags.length > 0 &&
                        docxTags.map((item, i) => <p key={item.id}>{item.name}</p>)}
                    </div>
                    <Button className={styles.syncBtn} onClick={() => this.setMappingBoxTrue()}>
                      映射设置列表
                    </Button>
                  </TabPane>
                  <TabPane tab={'权限设置'} key="3">
                    ...
                    />
                    <div className={styles.tagsBox2+' '+styles.tagsBox+' '+styles.docxtab}>
                      {data.length > 0 ? (
                        <div style={{marginBottom: '5px',paddingLeft: '20px'}}>
                          <p>
                            <span style={{width:'60%'}}>标签名称</span>
                            <span style={{width:'40%'}}>用户数量</span>
                          </p>
                          {data.map((item, idx) => (
                            <p key={item.id || idx}>
                              <span style={{width:'60%'}}>{item.directoryName}</span>
                              <span style={{width:'40%'}}>{item.userList.length}</span>
                            </p>
